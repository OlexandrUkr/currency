...
def test_get_contact_us(client):
    response = client.get('/currency/message/create/')
    assert response.status_code == 200


def test_post_empty_form_200(client):
    response = client.post('/currency/message/create/')
    assert response.status_code == 200
...
def test_post_invalid_email_200(client):
    ...
    response = client.post('/currency/message/create/', data=payload)
    assert response.status_code == 200
...
def test_post_valid_data(client, mailoutbox, settings):
    ...
    payload = {
        'name': 'Taras',
        'email_from': 'taras@example.com',
        'subject': 'This field is required.',
        'message': 'This field is required.'
    }
    response = client.post('/currency/message/create/', data=payload)
    assert response.status_code == 302
    assert response['location'] == '/currency/message/list/'
